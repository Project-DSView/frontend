...
class SinglyLinkedListService {
  ...
  // Search by Value
  async searchByValue(value: string): Promise<SinglyLinkedListExecutionStep[]> {
    const steps: SinglyLinkedListExecutionStep[] = [];
    ...
    if (this.state.nodes.length === 0) {
      steps.push(
        createExecutionStep(
          `LinkedList ว่าง - ไม่พบค่า ${value}`,
          `LinkedList ว่าง - คืนค่า -1`,
          1000,
        ),
      );
    } else {
      ...
      for (let i = 0; i < this.state.nodes.length; i++) {
        steps.push(
          createExecutionStep(
            `ตรวจสอบค่า ${this.state.nodes[i]} ที่ตำแหน่ง ${i}`,
            `ตรวจสอบ current.data = ${this.state.nodes[i]}`,
            600,
          ),
        );

        if (this.state.nodes[i] === value) {
          steps.push(
            createExecutionStep(
              `พบค่า ${value} ที่ตำแหน่ง ${i}`,
              `พบค่า ${value} ที่ตำแหน่ง ${i}`,
              1000,
            ),
          );
        ...
        if (i < this.state.nodes.length - 1) {
          steps.push(
            createExecutionStep(`ไปยัง node ถัดไป`, `current = current.next, position++`, 600),
          );
        }
      }
  ...
  // Search by Position
  async searchByPosition(position: number): Promise<SinglyLinkedListExecutionStep[]> {
    const steps: SinglyLinkedListExecutionStep[] = [];

    steps.push(
      createExecutionStep(
        `กำลังค้นหาค่าที่ตำแหน่ง ${position}`,
        `เรียกใช้ ll.search_by_position(${position})`,
        1000,
      ),
    );
    ...
    if (position < 0 || position >= this.state.nodes.length) {
      steps.push(
        createExecutionStep(
          `ตำแหน่ง ${position} ไม่ถูกต้อง`,
          `ตำแหน่ง ${position} ไม่ถูกต้อง - คืนค่า None`,
          1000,
        ),
      );
      return steps;
    ...
    steps.push(
      createExecutionStep(
        `พบค่า ${this.state.nodes[position]} ที่ตำแหน่ง ${position}`,
        `พบค่า ${this.state.nodes[position]} ที่ตำแหน่ง ${position}`,
        1000,
      ),
    );
