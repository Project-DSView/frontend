...
class DoublyLinkedListService {
  ...
  // Search by Value
  async searchByValue(value: string): Promise<DoublyLinkedListExecutionStep[]> {
    const steps: DoublyLinkedListExecutionStep[] = [];

    steps.push(
      createExecutionStep(
        `กำลัง Search ค่า ${value}`,
        `เรียกใช้ dll.search_by_value(${value})`,
        1000,
      ),
    );
  ...
  // Search by Position
  async searchByPosition(position: number): Promise<DoublyLinkedListExecutionStep[]> {
    const steps: DoublyLinkedListExecutionStep[] = [];

    steps.push(
      createExecutionStep(
        `กำลัง Search ที่ตำแหน่ง ${position}`,
        `เรียกใช้ dll.search_by_position(${position})`,
        1000,
      ),
    );
    ...
    if (position < 0 || position >= this.state.nodes.length) {
      ...
      const value = this.state.nodes[position];
      steps.push(
        createExecutionStep(
          `พบค่า ${value} ที่ตำแหน่ง ${position}`,
          `พบค่า ${value} ที่ตำแหน่ง ${position}`,
          1000,
        ),
      );
