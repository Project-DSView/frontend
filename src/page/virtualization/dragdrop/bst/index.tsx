...
const DragDropBST = () => {
  const { state, addOperation, updateOperation, removeOperation, clearAll, reorderOperation, updateBSTState } =
    useDragDropBST();
  ...
  /* =============================
        UPDATE INPUT VALUE
  ============================== */
  const updateOperationValue = (id: string, value: string) => {
    const op = state.operations.find((o: any) => o.id === id);
    if (op?.type === 'insert' && value && isNaN(parseFloat(value))) return;
    updateOperation(id, { value });
  };

  /* =============================
        AUTO BUILD BST + CODE GEN
  ============================== */
  React.useEffect(() => {
    let root: BSTNode | null = null;

    state.operations.forEach((op: any) => {
      if (op.value) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 px-4 py-5 dark:bg-gray-900 md:px-8">
      ...
      {/* ==========================================
          ⭐ BST OPERATIONS LIST (ย่อให้เล็กลง)
      =========================================== */}
        ...
        <div className="flex flex-wrap gap-2">
          {bstDragComponents.map((component) => (
            <div
              ...
              className="select-none cursor-grab rounded-full border px-3 py-1 text-xs font-medium transition hover:opacity-80"
              style={{
      ...
      {/* ==========================================
         ⭐ TWO COLUMNS: DROPZONE | VISUALIZATION (เตี้ยลง + scroll ภายใน)
      =========================================== */}
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        {/* Left: DROPZONE */}
        <div className="flex flex-col rounded-xl border border-dashed border-gray-300 bg-white p-3 shadow-sm dark:bg-gray-800 lg:h-[520px]">
          <div className="mb-2 flex items-center justify-between">
            ...
            <button
              ...
              className="rounded-md bg-gray-700 px-3 py-1 text-xs font-medium text-white hover:bg-gray-600 transition"
            >
              Clear
            </button>
          </div>
