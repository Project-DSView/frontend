...
import { StackRealtimeService } from '@/services';
...
import { StackData as RealtimeStackData, ParsedCode } from '@/types/realtime/Stack.types';

const useRealtimeStack = (code: string) => {
  const [data, setData] = useState<RealtimeStackData>({
    elements: [],
    count: 0,
  });
  ...
  const parseCode = useCallback(
    (code: string): ParsedCode => {
      try {
        const service = new StackRealtimeService(data);
  ...
  const executeCode = useCallback(
    (code: string, securityStatus: SecurityStatus): SecurityExecutionResult<RealtimeStackData> => {
      const service = new StackRealtimeService({ elements: [], count: 0 });

      // Parse code to get operations and stack variables
      const parseResult = service.parseCode(code);

      if (parseResult.isValid && parseResult.operations.length > 0) {
        // Execute operations with stack variables
        const result = service.executeOperations(
          parseResult.operations,
          parseResult.stackVariables,
        );
        return {
          success: true,
          data: result,
