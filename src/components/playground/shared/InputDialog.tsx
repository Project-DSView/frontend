...
const InputDialog: React.FC<InputDialogProps> = ({
  ...
  const parseInputValues = (text: string): string[] => {
    ...
    if (text.includes('\n')) {
      ...
      // Split by commas
      values = text.split(',').map(v => v.trim()).filter(v => v.length > 0);
    } else {
      // Single value
  ...
  const isValid = parsedValues.length >= totalInputs && parsedValues.every(v => v.trim().length > 0);
      ...
      <AlertDialogContent className="sm:max-w-lg">
        <AlertDialogHeader>
          ...
          <AlertDialogDescription className="text-gray-600 dark:text-gray-300">
            ระบบตรวจพบ input() {totalInputs} แห่ง แต่อาจมีมากกว่านี้หากมีการวนลูป<br/>
        ...
        <div className="space-y-4">
          {prompts && prompts.length === 1 && prompts[0].prompt && (
             <div className="bg-blue-50 p-3 rounded-md border border-blue-100 dark:bg-blue-900/20 dark:border-blue-800">
          ...
          <div className="space-y-2">
            <Label htmlFor="input-textarea" className="text-sm font-medium">
              {prompts && prompts.length === 1 ? 'Value' : `Input Values (Filled ${parsedValues.length})`}
            </Label>
            <Textarea
              id="input-textarea"
              value={inputText}
              ...
              placeholder={prompts && prompts.length === 1 ? `Enter value for '${prompts[0].prompt}'` : `Example: 5, 10, area\nOr:\n5\n10\narea`}
              autoFocus
              className="font-mono min-h-[120px] resize-none border-gray-300 focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-800 dark:border-gray-600 dark:text-gray-100"
              rows={4}
            />
            <div className="flex items-center justify-between">
              <p className="text-xs text-gray-500 dark:text-gray-400">
                {prompts && prompts.length === 1 ? 'Press Ctrl+Enter to submit' : 'Separate with commas or newlines • Ctrl+Enter to submit'}
              </p>
