  ...
  return (
    <div className="mb-4 rounded-md bg-white p-3 shadow-sm dark:bg-gray-800">
      ...
      <div className="mb-2 flex items-center justify-between">
        <div>
          <h2 className="text-sm font-semibold text-gray-800 dark:text-gray-100">Step Control</h2>
          <span className="text-xs text-gray-500">
            {currentStepIndex + 1} / {operations.length}
          </span>
        </div>
        {isAutoPlaying && (
          <span className="animate-pulse text-xs font-medium text-green-600">Playing…</span>
        )}
      </div>
      ...
      <div className="mt-3 rounded-md bg-blue-50 px-3 py-2 text-xs dark:bg-blue-900/40">
        ...
        {currentOperation ? getStepDescription(currentOperation) : 'No operation'}
      </div>
    </div>
  );
};
