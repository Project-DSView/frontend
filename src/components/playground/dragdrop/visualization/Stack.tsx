...
import ZoomableContainer from '@/components/playground/shared/ZoomableContainer';
    ...
    const renderStackElement = (value: string, index: number, stackLength: number) => {
      ...
      return (
        <div key={`${value}-${index}`} className="relative flex flex-col items-center">
          ...
          <div
            className={`
              flex h-16 w-16 items-center justify-center bg-gray-100 dark:bg-gray-700 shadow-lg transition-all 
              ${isHighlighted && isAnimating ? 'scale-110 ring-4 ring-blue-400 dark:ring-blue-500' : ''}
              ${isTop ? 'border-2 border-blue-500 dark:border-blue-400 ring-2 ring-blue-300 dark:ring-blue-600' : ''}
            `}
          >
            <span
      ...
      <div className="flex flex-col items-center">
        ...
        <div className="flex min-h-[200px] flex-col items-center justify-end">
          {stackElements.length === 0 ? (
            <div className="flex h-24 w-32 items-center justify-center border-r-2 border-b-2 border-l-2 border-dashed border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800">
              <div className="text-center text-gray-500 dark:text-gray-400 text-xs font-semibold">
                Empty
              </div>
            </div>
          ) : (
            <div className="relative w-32">
              ...
              <div className="flex flex-col-reverse">
                {stackElements.map((element, index) => (
                  <div key={`${stackId}-${element}-${index}`}>{renderStackElement(element, index, stackElements.length)}</div>
                ))}
              </div>
            </div>
          )}
        ...
        >
          {showMultipleStacks ? (
            ...
            <div className="flex min-h-[200px] flex-col items-center justify-end p-6">
              {elements.length === 0 ? (
                <div className="flex h-32 w-40 items-center justify-center border-r-2 border-b-2 border-l-2 border-dashed border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800">
                ...
                <div className="relative w-36">
                  ...
                  <div className="flex flex-col-reverse">
                    {elements.map((element, index) => (
                      <div key={`${element}-${index}`}>{renderStackElement(element, index, elements.length)}</div>
                    ))}
                  </div>
                </div>
              )}
