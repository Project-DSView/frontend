      ...
      <ZoomableContainer className="min-h-[400px] rounded-lg bg-gray-50" showControls>
        {nodes.length === 0 ? (
          ...
          <div className="relative h-full min-h-[400px] w-full">
            ...
            {/* ===== NODES ===== */}
            {nodes.map((node) => {
              const outgoing = Array.isArray(node.outgoingEdges)
                ? node.outgoingEdges
                : [];
              const incoming = Array.isArray(node.incomingEdges)
                ? node.incomingEdges
                : [];

              return (
                <GraphNode
                  key={node.id} // ✅ key อยู่ตรง .map()
                  node={{
                    ...node,
...
DirectedGraphDragDropVisualization.displayName =
