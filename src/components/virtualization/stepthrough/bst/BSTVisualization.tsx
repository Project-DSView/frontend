...
>(({ steps, currentStepIndex, data, isRunning }, ref) => {
  ...
  // Extract root from data or steps data
  const root = useMemo(() => {
    // First try to get root from data (from hook)
    if (data && 'root' in data && data.root) {
      console.log('BST Root from data:', data.root);
      return data.root as BSTNode;
    }
    ...
    if (steps.length > 0 && currentStepIndex < steps.length) {
      ...
      if (state.step_detail) {
        ...
        if (stepDetail.after_tree) {
          console.log('BST Root from step_detail:', stepDetail.after_tree);
          return convertToBSTNode(stepDetail.after_tree);
        }
      }
    }

    console.log('BST Root not found');
    return null;
