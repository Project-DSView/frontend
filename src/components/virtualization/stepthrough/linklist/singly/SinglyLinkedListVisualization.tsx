...
>(({ steps, currentStepIndex, linkedListData, isRunning }, ref) => {
  ...
  const nodesRef = useRef(linkedListData.nodes);
  const [nodes, setNodes] = useState(linkedListData.nodes);

  // Update nodes when linkedListData.nodes actually changes
  useEffect(() => {
    if (JSON.stringify(nodesRef.current) !== JSON.stringify(linkedListData.nodes)) {
      nodesRef.current = linkedListData.nodes;
      setNodes(linkedListData.nodes);
    }
  ...
  return (
    <div ref={ref} className="rounded-lg bg-white p-6 shadow">
      ...
      <div className="mt-4 flex space-x-6 text-sm text-gray-600">
        ...
        <div>
          <span className="font-semibold">Head Value:</span> {linkedListData.head || 'None'}
        </div>
        <div>
          <span className="font-semibold">Tail Value:</span> {linkedListData.tail || 'None'}
        </div>
      </div>
